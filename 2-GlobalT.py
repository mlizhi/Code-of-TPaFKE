...
def load_triples(file): #同时考虑关系的正反方向
    '''
    load triples
    输入格式(h, r, t)
    '''
...
def generate_path(): #遍历所有实体和关系，生成路径，并计算这些路径的频率以及与特定关系的共现频率
    '''
...
def proir_path_confidence(in_file, out_file): #生成路径置信度
    '''
...
if __name__ == '__main__':
    ...
    load_triples(in_path + 'data_with_neg.tsv')#正例和负例
    print('[INFO] 数据载入完成')
    generate_path()
    proir_path_confidence(in_path + "data_with_neg.tsv", in_path + "pos_with_neg_PP_conf.txt") # file_confidence
    work(in_path + "data_with_neg.tsv", in_path + 'pos_with_neg_pra.txt') # file_pra.txt