...
private:
    ...
    void bfgs()
    {
        ...
        for (int epoch=0; epoch<nepoch; epoch++)
        {
            ...
            // 保存向量
            FILE* f2 = fopen(("./output_results/relation2vec."+version).c_str(),"w");
            FILE* f3 = fopen(("./output_results/entity2vec."+version).c_str(),"w");
            for (int i=0; i<relation_num; i++)
            {
                for (int ii=0; ii<n; ii++)
...
void prepare()
{
    ...
    */
    FILE* f1 = fopen("./training_data/entity_id.txt", "r");
    FILE* f2 = fopen("./training_data/relation_id.txt", "r");
    int x;
    ...
    //int c;
    //c = getchar();
    FILE* f_kb = fopen("./training_data/train2id.txt", "r");
    if (f_kb == NULL) {
        perror("Error opening train2id.txt");
        exit(1); // 或其他错误处理
    }
    int head, tail, rel;
    cout << "Loading training data..." << endl;
    while (fgets(buf, sizeof(buf), f_kb) != NULL) {
        if (sscanf(buf, "%d\t%d\t%d", &head, &rel, &tail) == 3) {
            // 检查头实体和尾实体是否存在
            if (id2entity.count(head) == 0) {
                cout << "miss head entity: " << head << endl;
            } else if (id2entity.count(tail) == 0) {
                cout << "miss tail entity: " << tail << endl;
            } else if (id2relation.count(rel) == 0) {
                cout << "miss relation: " << rel << endl;
            } else {
                left_entity[rel][head]++;
                right_entity[rel][tail]++;
                train.add(head, tail, rel);
            }
        }
    }
    fclose(f_kb);
