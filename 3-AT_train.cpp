...
//normal distribution
double rand(double min, double max) //生成一个介于min和max之间的随机数,介于0和RAND_MAX之间
{
    return min+(max-min)*rand()/(RAND_MAX+1.0); 
}
double normal(double x, double miu,double sigma) //计算正态分布的概率密度函数值,x是变量，miu是均值，sigma是标准差
{
    return 1.0/sqrt(2*pi)/sigma*exp(-1*(x-miu)*(x-miu)/(2*sigma*sigma));
}
double randn(double miu,double sigma, double min ,double max) //生成在特定范围（min到max）内的服从正态分布的随机数
{
    double x,y,dScope;
    do{
        x=rand(min,max);
    ...
    }while(dScope>y); //当dScope小于或等于正态分布下的概率密度y时，才接受这个x值
    return x;
}
...
double vec_len(vector<double> &a) //计算一个double类型向量的长度（或欧几里得范数）
{
	double res=0;
...
private:
    ...
    double norm(vector<double> &a) //用于归一化向量
    {
        double x = vec_len(a);
        if (x>1)
    ...
    int rand_max(int x) //在训练过程中随机选择样本或进行其他随机操作
    {
        int res = (rand()*rand())%x;
        while (res<0)
            res+=x;
	...
	double calc_path(int r1,vector<int> rel_path)		//calc ||p-r||计算给定路径和特定关系之间的差异值
    {
        double sum=0;
        for (int ii=0; ii<n; ii++)
		{
...
void prepare()
{
    ...
    FILE* f1 = fopen("./2-data_with_neg/entity_id.tsv","r");
	FILE* f2 = fopen("./2-data_with_neg/relation_id.tsv","r");
	int x;
