...
def read_triples_id(file):
    '''
    Read triple file (h, r, t, confidence) but only store (h, r, t)
    '''
    triples = []
    with open(file) as f:
        lines = f.readlines()
        for line in lines:
            line = line.strip()  # Removes any leading/trailing whitespace including newline
            h, r, t, _ = line.split('\t')  # Use _ to ignore the confidence value
            triples.append((h, r, t))  # Store only the triple (h, r, t)
    f.close()
    return triples
...
def candidate_triples(pos_triples):
    ...
    for ent in all_entities:
        parts = ent.split(':')
        # 由于实体字符串格式，我们可以直接获取第二部分作为类型
        type_ = parts[1] if len(parts) > 1 else 'unknown'  # 如果没有第二部分，则标记为 'unknown'
    ...
    for rel, triples in rel2triples.items():
        ...
        for example in triples:
            parts = example[2].split(':')
            type_ = parts[1] if len(parts) > 1 else 'unknown'  # 同样处理尾实体的类型
